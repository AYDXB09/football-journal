...
| AI Feedback | Anthropic Claude API via Next.js API Route Handlers (server-side only, key never on client) |
| Hosting | Vercel |
...
- All Anthropic API calls made server-side via Next.js API Route Handlers (`/app/api/ai/...`)
...
- **Log Match** — full match entry: competition + stage selectors, multiple position rows with from/to minutes (position-aware rating dimensions for CB vs ST etc), total match time vs time played, mood, performance ratings, @mention reflections (went well / improve / key moment / coach feedback), video moment links with type tags and timestamps
...
- **⚙️ Setup** — manage seasons, clubs, teams, competitions, teammates via modal forms in dependency order
